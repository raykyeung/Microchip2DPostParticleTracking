function lines = B2Khoughlines(varargin)
...
%   Copyright (c) 2023, Raymond Yeung
...
numlines = 0; 
lines = initializeEmptyStructArray();
...
for k = 1:size(peaks,1)
  ...
  for p = 1:length(idx) - 1
    ...
    if linelength_sq >= minlength_sq
      ...
      lines(numlines).votes = size(xy,1);
    end
  end
end

function lines = initializeEmptyStructArray()
